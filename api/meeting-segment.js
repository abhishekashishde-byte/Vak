import { domainKeywords, domainPrompt, publicDomain, resolveDomain, transcriptionDomainPrompt } from './_domain.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const keywords = domainKeywords(initialDomain, { includeUniversal: true, limit: 24 })
    ...
    const transcriptionPrompt = [
      ...
      specialistHint,
      keywords.length ? `Possible specialist vocabulary includes: ${keywords.join(', ')}.` : '',
      context ? `Previous meeting context for names and continuity only; do NOT repeat it: ${context}` : '',
    ].filter(Boolean).join(' ')
    ...
    if (!transcript) return res.status(200).json({ transcript: '', translation: '', domain: publicDomain(initialDomain) })

    const finalDomain = resolveDomain(`${context}\n${transcript}`, req.body?.domain)
    ...
    const specialistTranslation = domainPrompt(finalDomain)
    if (specialistTranslation) instructions += `\n\n${specialistTranslation}`
    ...
    return res.status(200).json({ transcript, translation, domain: publicDomain(finalDomain) })
  } catch (error) {
